...
export const MedicalImageViewer: React.FC<MedicalImageViewerProps> = ({
  ...
  }, [frameUrls, zoom, panOffset, windowWidth, windowLevel, measurements, activeMeasurement, annotations, activeAnnotation, previewPoint, selectedMeasurementId, hoveredMeasurementId, editingLabelId, editingLabelText, activeTool, leaderStep])
    ...
    try {
      switch (format) {
        case 'pdf':
          ...
          break
        case 'docx':
          const textReport = await reportExportService.exportToText(reportData)
          const blob = new Blob([textReport], { type: 'text/plain' })
          const url = URL.createObjectURL(blob)
          const link = document.createElement('a')
          link.href = url
          link.download = `Report_${reportData.studyInfo.patientName.replace(/[^a-zA-Z0-9]/g, '_')}.txt`
          link.click()
  ...
  }, [currentReport, measurements, metadata, studyInstanceUID, reportExportService])
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      const shortcuts = ['w', 'z', 'p', 'l', 'a', 't', 'r', ' ', '?', '1', '2', '3', '4', 'f', 'i', 'm']
