...
import { createVTKRenderer, preloadVTK, isVTKLoaded } from '../utils/volumeRendererVTKLazy'
...
export function useVolumeRenderer({
  ...
  useEffect(() => {
    ...
    if (check.canRender) {
      ...
      // Preload VTK.js in the background if WebGL is supported
      // This reduces the delay when user activates 3D mode
      console.log('🔄 Preloading VTK.js in background...')
      preloadVTK().catch(err => {
